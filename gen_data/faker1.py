...
from config import Type1Config


class ImageFakerType1(ImageFaker):
    ...
    def _init_config(self):
        ...
        for k in dir(Type1Config):
            if not k.startswith('__'):  # Ignore dunder and methods
                v = getattr(Type1Config, k)
                setattr(self, k, v)


    def find_large_trailing_space_rows(self, bb_rows, mean_row_h, mean_word_dist, im_w):
        cand_rows = []
        for row_idx, row in enumerate(bb_rows):
            if not is_valid_row(row, mean_row_h, dist_threshold=mean_word_dist):
                continue
            last_bb = row[-1]
            if last_bb[2] <= 3/4 * im_w:
                cand_rows.append(row_idx)
        return cand_rows


    def paste_one_row_trailing_space(self, bg, row, all_pr_hw_bbs):
        hw_bbs = []
        # calc num words to paste
        mean_word_width = np.mean([bb[2]-bb[0] for bb in row])
        traling_space = bg.width - row[-1][2]
        max_words = int(traling_space/mean_word_width)
        ...
        # paste
        last_pr_bb = row[-1]  # last printing box
        paste_xmin = last_pr_bb[2] + default_offset
        ...
        for roi_idx, (roi, text) in enumerate(zip(rois, texts)):
            ...
            paste_xmin += roi.width + default_offset
    ...
    def fake(
        ...
        """
            Paste hw boxes to trailing space of rows
        """
        ...
        # 2. find rows with large trailing space
        cand_rows = self.find_large_trailing_space_rows(bb_rows, mean_row_h, mean_word_dist, im_w)
        if len(cand_rows) == 0:
            print('Skipping: No rows with large trailing space')
            return None, [], orig_all_pr_hw_bbs
        ...
        for paste_row_idx in paste_row_indexes:
            ...
            new_bg, hw_bbs = self.paste_one_row_trailing_space(deepcopy(bg), paste_row, deepcopy(all_pr_hw_bbs))
